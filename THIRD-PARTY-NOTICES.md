...
No third-party source tree is copied into this repository. The entries below
are either small adaptations written into our own modules, or designs we
reimplemented from scratch. Two exceptions are data rather than code, both MIT
and both recorded in the table: `tests/fixtures/upstream/` holds excerpts of the
JSON that `npm run sync:endpoints` and `npm run sync:user-agents` read, kept so
their tests run offline, and each generated module carries the attribution in
its own header.
...
| [fa0311/TwitterInternalAPIDocument](https://github.com/fa0311/TwitterInternalAPIDocument) | MIT | The GraphQL operation table (query ID, operation name, type), the feature-switch values and field-toggle names x.com's own client sends, and the v1.1 REST dispatch table, from `docs/json/GraphQL.json` and `docs/json/v1.1.json` on the `develop` branch. Regenerated by `npm run sync:endpoints` | `src/scrapers/twitter/http/x-endpoints.generated.js`, `scripts/sync-x-endpoints.mjs`, `tests/fixtures/upstream/x-endpoints/` |
| [fa0311/latest-user-agent](https://github.com/fa0311/latest-user-agent) | MIT | Current browser User-Agent strings and the request headers that accompany them, including the `Sec-CH-UA` brand lists, from `output.json` and `header.json`. Regenerated by `npm run sync:user-agents` | `src/client/auth/userAgents.generated.js`, `scripts/sync-user-agents.mjs`, `tests/fixtures/upstream/user-agents/` |
...
| [vercel-labs/skills](https://github.com/vercel-labs/skills) | MIT | Nothing from their CLI: `xactions skills install` stays. What we took is the discovery contract it implements, the [Agent Skills specification](https://agentskills.io/specification), so any third-party installer resolves our catalogue. Our tree was audited field by field against that spec | `skills/*/SKILL.md`, `docs/skills.md` |
| [karanb192/reddit-mcp-buddy](https://github.com/karanb192/reddit-mcp-buddy) | MIT | Shipping the MCP server as a drag-and-drop `.mcpb` release artifact whose manifest collects its secrets at install time, so nobody hand-edits a JSON config. Written against Anthropic's published MCPB manifest spec and packed with the official `@anthropic-ai/mcpb` CLI | `scripts/build-mcpb.mjs`, `.github/workflows/release-mcpb.yml` |
| [cosinusalpha/webctl](https://github.com/cosinusalpha/webctl) | No LICENSE file, ideas only | Running a CLI lane beside the MCP lane, and telling the agent when one shell call is cheaper than loading a large tool list. Their comparison is the argument for the lane; the decision table, the flags and the recipes are ours | `AGENTS.md`, `docs/mcp-setup.md` |
