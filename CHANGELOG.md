...
- **`https://xactions.app/mcp` sells its tools by the call.** A remote MCP server over Streamable HTTP that any client can use on the first try: no account, no API key, no OAuth round trip. `xactions_docs` and `x_video` are free; `x_profile`, `x_post`, `x_posts` and `x_thread` answer with x402 terms, and an agent that pays a fraction of a cent in USDC on Solana or Base gets the result with the settlement receipt in `_meta["x402/payment-response"]`. `tools/list` carries the price, so an agent budgets before it calls instead of discovering the cost by being refused.
- **[`@xactions/x402-mcp`](packages/x402-mcp/README.md)** — the payment layer, published on its own. MCP gave agents a way to call tools and nobody a way to charge for them, so every hosted MCP server is either free or behind a key an agent cannot sign up for. `createToolPaymentGate()` adds payment to a server you already have in three lines; `createPaidMcpServer()` is a complete Streamable HTTP server as one `fetch` handler; `PaidMcpClient` buys from one, bounded by a per-call limit, a session budget and a payee allowlist that are all checked before a signer sees the terms. Zero dependencies, so it runs on Workers, Deno, Bun and Node. Terms are intersected with the facilitator's own `/supported` list and its `extra` merged in, which is what supplies Solana's fee payer, and both x402 protocol generations are spoken because a v1 indexer handed a CAIP-2 chain id rejects the challenge outright. 35 tests run the real client against the real server.
